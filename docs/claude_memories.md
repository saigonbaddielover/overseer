...
`jq -r .version ~/.claude/plugins/marketplaces/saigonbaddielover/plugins/overseer/.claude-plugin/plugin.json`.

**`autoUpdate: true` does not help the session you are in.** The `saigonbaddielover` marketplace entry in
...
auto-updates). Diagnose with `ls ~/.claude/plugins/cache/saigonbaddielover/overseer/*/.in_use/ && echo $CLAUDE_PID`.
