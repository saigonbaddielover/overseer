...
| `discover [--write] [--no-probe] [--no-tailscale] [--os NAME] [-u USER] [-t secs]` | **Remote (SSH), auto-inventory.** Work out which remote machines this box can reach and how, so you (and the user) don't hand-maintain a hosts file. Enumerates candidates from **tailnet peers** (`tailscale status`) **and** non-wildcard `Host` aliases in `~/.ssh/config` + its `Include`d files, resolves each via **`ssh -G`** (effective user/hostname/identity — ssh config is the source of truth, overseer never guesses a user), probes for `OS` + drive-ability, and classes each row: **`drivable`** (Linux, tmux+jq), **`needs-deps`** (reachable, missing tmux/jq → `provision`), **`windows`** (a `win <host>` target), **`gap`** (ssh auth failed → you must get the `user@host`/key from the user), **`unreachable`**, **`self`** (skipped). **`--write`** saves the drivable `user@ip` into `$XDG_CONFIG_HOME/overseer/hosts` in a managed block (hand-written lines kept), the file `hosts`/`fleet --hosts` read first. `--no-probe` = static resolve, `--no-tailscale` = ssh-config only. **Agentic use:** run it when the user asks "what can I reach / set up my fleet" or before a fleet op with no inventory; then for every `gap`/`unreachable` row, *ask the user* for that machine's user/key (or how to reach it over LAN/tailnet) and guide `provision`/ssh-key setup, and only `--write` once the reachable set is confirmed. | read-only unless `--write` (writes the hosts file) |
