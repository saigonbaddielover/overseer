...
| `wait <target> [timeout]` | **Agent pane (Claude or Codex).** Block until the target **drains to idle** — its current turn finishes *and* any queued message behind it has run — or return early with the question if the agent stops at an interactive prompt awaiting your input. Correctly waits through a **compaction** with a queued message (a compacting agent reads *idle* in the transcript, so a naive check would return early) and through a text-only turn (no `tool_use`) — give it a generous timeout, since a compaction can take minutes. If the turn is **interrupted** (someone hit Ctrl-C in that pane) it returns `idle` within seconds instead of blocking to the timeout: Claude writes nothing on an interrupt, so the wait ends when the pane's screen stops moving entirely across several consecutive samples — a live turn always moves, either its spinner or the reply text it is streaming. | read-only |
