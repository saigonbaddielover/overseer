...
| `usage [--json] [target]` | **Account quota + context, kept apart on purpose.** Per harness: how much of each usage window is spent (`5h`/`7d` for Claude, the rolling window for Codex) and when it resets, then — separately, labelled *informational, never a fault* — the session's context fill. **Do not report a filling context as a problem**: both harnesses auto-compact, so it is not a failure mode; **quota at 100% is** the thing that stops work. No target = this machine's account; a target reads that pane's session. `--json` for scripts. Claude publishes `rate_limits` **only to a statusline**, so wire the collector once: `usage --install` (chains any existing statusline; `--install --here` if a project `statusLine` overrides the user one). `quota n/a` means the account has no subscription window — normal on a third-party backend (Bedrock/Vertex/API key/proxy), not an error. | read-only (`--install`/`--uninstall` edit `settings.json`) |
