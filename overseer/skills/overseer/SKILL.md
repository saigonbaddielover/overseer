...
| `discover [--write] [--no-probe] [--no-tailscale] [--etc-hosts] [--os NAME] [-u USER] [-t secs]` | **Remote (SSH), auto-inventory.** Work out which remote machines this box can reach and how, so you don't hand-maintain a hosts file. Enumerates candidates from **every source**: tailnet peers (`tailscale status`), `Host` aliases in `~/.ssh/config` **+ `/etc/ssh/ssh_config` + every `Include`**, `known_hosts` (unhashed read directly; hashed can't be reversed — one-way HMAC — but every candidate is cross-checked with `ssh-keygen -F`, tagging hosts you've connected to), shell history ssh targets (bash/zsh/fish), and docker `ssh://` contexts; `--etc-hosts` also scans `/etc/hosts` (off by default — adblock hosts files flood it). A **`SOURCE`** column shows provenance. Resolves each via **`ssh -G`** (ssh config is the source of truth; overseer never guesses a user), but an explicit `user@` in a source is used as the login **hint**. Probes `OS` + drive-ability and classes each: **`drivable`** (Linux, tmux+jq), **`needs-deps`** (reachable, missing tmux/jq → `provision`), **`windows`** (`win <host>` target), **`gap`** (ssh auth failed → get the `user@host`/key from the user), **`unreachable`**, **`self`** (skipped). **`--write`** saves the drivable `user@ip` into `$XDG_CONFIG_HOME/overseer/hosts` in a managed block (hand-written lines kept), read first by `hosts`/`fleet --hosts`. `--no-probe` = static resolve, `--no-tailscale` = skip tailnet. **Agentic use:** run it when the user asks "what can I reach / set up my fleet" or before a fleet op with no inventory; for every `gap`/`unreachable` row, *ask the user* for that machine's user/key (or how to reach it over LAN/tailnet) and guide `provision`/ssh-key setup, and only `--write` once confirmed. | read-only unless `--write` (writes the hosts file) |
