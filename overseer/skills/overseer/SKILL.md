...
| Command | Effect | Safe? |
|---|---|---|
| `list [--all]` | List tmux panes running an agent + its **HARNESS** (claude or codex): session, pane, pids, cwd, and **PEER** — the harness name a claude session is addressable by, `-` when it has none; a target may be given by that PEER name as well as by `%N` or session name. `--all` lists **every** tmux pane and its foreground command — use it to find shell panes to target. | read-only |
...
10. **Prefer the harness peer channel over keystrokes for a local Claude target.** `list` shows a
   **PEER** name for every claude session that exposes one, and `chat`/`send` **refuse** such a target
   ...
   the keyboard. Every keystroke delivery is prefixed with the sender's own PEER name, so a
   Codex/shell/remote target still knows who wrote to it — that prefix is a convention, not proof.
