...
| `discover [--write] [--no-probe] [--no-tailscale] [--os NAME] [-u USER] [-t secs]` | **Remote (SSH), auto-inventory.** Find the remote machines this box can actually reach and how, instead of hand-maintaining a hosts file. It enumerates candidates from two sources — every **tailnet peer** (`tailscale status`) and every non-wildcard `Host` alias in `~/.ssh/config` **and its `Include`d files** — resolves each through **`ssh -G`** (so the effective user, hostname and identity come straight from your ssh config, the single source of truth), then probes each to fill `OS` and drive-ability. Every row is classed: **`drivable`** (Linux, `tmux`+`jq`, ssh ok), **`needs-deps`** (Linux, reachable, missing `tmux`/`jq` — run `provision`), **`windows`** (a `win <host>` target, not a fleet host), **`gap`** (ssh could not authenticate — overseer does *not* guess a user; tell it the right `user@host` or key), **`unreachable`**, or **`self`** (this box, skipped). **`--write`** persists the drivable `user@ip` lines into `$XDG_CONFIG_HOME/overseer/hosts` inside a **managed block** — hand-written lines outside the block are preserved — which is exactly the file `hosts`/`fleet --hosts` read first, so one `discover --write` wires the fleet. `--no-probe` resolves statically (offline, no ssh), `--no-tailscale` skips the tailnet source, `--os`/`-u`/`-t` as in `hosts`. |
