...
| Command | Effect |
|---|---|
| `list [--all]` | List agent panes led by **PEER** — the harness name a claude session is addressable by (`-` when it has none) — then session, pane, pids, **HARNESS** (claude/codex) and cwd. Every command takes that PEER name as its target, ahead of `%N` or a session name. `--all`: every pane + its foreground command. |
