...
| `wait <target> [timeout]` | **Agent (Claude/Codex).** Block until the current turn finishes — or return early if the agent stops at a prompt awaiting input. Correctly waits through a **compaction** with a queued message (which reads idle in the transcript) instead of returning early, and returns `idle` in seconds if the turn was **interrupted** (an interrupt leaves no trace in the transcript, so what ends the wait is the pane's screen not moving at all across several consecutive samples — a live turn always moves, either its spinner or the reply text it is writing) rather than blocking to the timeout. |
